...
async def run() -> None:
    ...
    process_parser = subparsers.add_parser('HARVESTER_INIT', help='Initialize a job with given inputs')
    process_parser.add_argument('--task_args',
                        action='store',
                        dest='job_args',
                        type=str,
                        help='JSON dump containing arguments for Harvester Processes')
    process_parser.add_argument('--persistence',
                        action='store_true',
                        dest='persistence',
                        default=False,
    ...
    process_parser = subparsers.add_parser('HARVESTER_MONITOR', help='Initialize a job with given inputs')
    process_parser.add_argument('--job_id',
                        action='store',
                        dest='job_id',
                        type=str,
                        help='Job ID string to query.')
    process_parser.add_argument('--persistence',
                        action='store_true',
                        dest='persistence',
                        default=False,
    ...
    async with grpc.aio.insecure_channel("localhost:50051") as channel:
        ...
        if args.action == "HARVESTER_INIT":
            if args.job_args: 
                ...
                if task_args.get('ingest'): s['task_args']['ingest'] = bool(task_args['ingest'])
            ...
            async for response in stub.monitorHarvester(
                s):
                print(response)
